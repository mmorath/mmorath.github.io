---
hide:
  - toc
---

# Assistance

L'aide est organisée en deux volets, selon le côté de Hecate où vous vous
trouvez :

<div class="grid cards" markdown>

-   :material-account-hard-hat: __Opérateurs__ · Saisie & Viewer

    ---

    Vous utilisez l'application de saisie ou Hecate Viewer sur le terrain —
    scanner, localiser, se connecter à un broker, regarder les actifs arriver
    sur la carte en direct.

    [:octicons-arrow-right-24: Assistance opérateurs](operator/index.md)

-   :material-tune-variant: __Administrateurs__ · Application admin

    ---

    Rédiger des profils, configurer le broker, publier et retirer des
    profils, gérer les identifiants.

    [:octicons-arrow-right-24: Assistance admin](admin/index.md)

</div>

## Soutenir le projet

Hecate est développé par une seule personne, en toute transparence — sans
backend, sans comptes, sans suivi. Si l'application vous évite de chercher un
outil, vous pouvez la faire vivre : chaque café se transforme en rouleaux
d'étiquettes, en matériel de test et en heures tranquilles qui gardent Hecate
libre et honnête.

[:material-coffee: Buy me a coffee](https://buymeacoffee.com/hecateHQ){ .md-button .md-button--primary }

<a href="https://buymeacoffee.com/hecateHQ">
  <img src="../../assets/hecate-bmac-qr.png" alt="QR code Buy Me a Coffee pour Hecate HQ" width="180">
</a>

## Contact

!!! note "Adresse de contact"
    **E-mail :** _à confirmer avant la soumission._

Lorsque vous signalez un problème, merci d'indiquer votre **appareil** et
votre **version d'iOS**, la **version de l'application**, ainsi que ce que
vous avez fait et ce que vous attendiez.
